...
mol load gro leaflet0_unwrap.gro
mol addfile leaflet0_unwrap.trr type trr first 0 last -1 step 1 waitfor all
...
mol load gro protein_unwrap.gro
...
for {set i 0} {$i < [expr $nresid-1]} {incr i} {
	...
	#set k [expr int([expr [expr $cx/$binsize]+0.5])]
        #set l [expr int([expr [expr $cy/$binsize]+0.5])]

	set patrix($k,$l) [expr {$patrix($k,$l) - 1}]
...
set fm [ open MSD.dat w+ ]
set fp [ open PRO.dat w+ ]
	
#from A^2 to nm^2
set unit 100

for {set i 0} {$i < [expr int($x_size/$binsize)]} {incr i} {
        for {set j 1} {$j < [expr int($y_size/$binsize)]} {incr j} {
